...
class Analycis:
    ...
    def getAvgPrice(self, region):
        try:
            ...
            # print(list(totalPrice)[0]["total_price"])
            totalPrice2 = list(totalPrice)[0]["total_price"]
            totalArea2 = list(totalNum)[0]["total_area"]
    ...
    # 获取户型数据（3 室 2 厅）
    def getRooms(self,region):
        areaPinYin = self.getPinyin(region=region)
        collection = self.zfdb[areaPinYin]
        results = collection.aggregate([{'$group': {'_id': '$rooms', 'weight': {'$sum': 1}}}])
        roomList = []
        weightList = []
    ...
    def showTreeMap(self, title, data):
        ...
        treemap.add("北京", data, is_label_show=True, label_pos='inside', label_text_size=19)
        treemap.render()

    # 展示条形图
    def showLine(self,chartitle, title, attr, value):
        from pyecharts import Bar
        bar = Bar(title)
        bar.add(chartitle, attr, value, is_convert=False, is_label_show=True, label_text_size=18, is_random=True,
...
# 户型统计
# (attr, value) = analycis.getRooms("燕郊")
# analycis.showLine("燕郊","户型统计", attr, value)

# 获取每月每平方米多少钱
data = analycis.getTotalAvgPrice()
print(data)
analycis.showTreeMap("北京各区单价：/元", data)
