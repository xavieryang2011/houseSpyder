...
class Analycis:
    ...
    # 获取户型数据（3 室 2 厅）
    def getRooms(self):
        results = self.zfdb.yj_bj.aggregate([{'$group': {'_id': '$rooms', 'weight': {'$sum': 1}}}])
        roomList = []
        weightList = []
    ...
    def showLine(self, title, attr, value):
        ...
        bar = Bar(title)
        bar.add("北京", attr, value, is_convert=False, is_label_show=True, label_text_size=18, is_random=True,
...
print(data)
analycis.showTreeMap("北京各区房租单价：平方米/月", data)
